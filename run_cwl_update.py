...
def first_crawl(**kwargs):
    os.system("python -m scrapy crawl cwl_update")
    delay(**kwargs)
...
def scheduler_increment_crawl():
    ...
    scheduler.add_job(start_crawl, 'interval', minutes=1, start_date=f'{nowday} 20:30:00', end_date=f'{nowday} 20:40:30')    #
    scheduler.start()
...
if __name__ == '__main__':
    ...
    # first_crawl(minutes=30)       # 采集全量数据
    # increment_crawl(minutes=30)     # 采集增量数据， 在sipder里面修改增量的页数

    from scrapy import cmdline
    cmdline.execute("scrapy crawl cwl_update".split())
